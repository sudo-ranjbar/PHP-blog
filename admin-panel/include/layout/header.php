...
if(!isset($_SESSION['email'])) {
    if (str_contains($path, 'pages')){
        header("Location:../auth/login.php?error=ابتدا وارد شوید");
    }else{
        header("Location:./pages/auth/login.php?error=ابتدا وارد شوید");
    }
    exit();
}
?>
...
<body>
    <header class="navbar sticky-top bg-secondary flex-md-nowrap p-0 shadow-sm">
        <a class="navbar-brand col-md-3 col-lg-2 me-0 px-3 fs-5 text-white" href="/php-pure-blog_project/admin-panel/index.php">پنل ادمین</a>
